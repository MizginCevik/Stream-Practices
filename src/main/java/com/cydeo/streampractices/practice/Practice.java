...
//      but it is a chance for it to not work if there is multiple objects that is matching with the condition.
...
//                .sorted(Comparator.comparing(Employee::getSalary).reversed())
//                .map(Employee::getSalary)
//                .distinct() //if there are two max salaries(exactly same amount), we have to use distinct
//                .skip(1)
//                .findFirst().get();

        return getAllEmployees().stream()
                .filter(employee -> employee.getSalary().compareTo(getMaxSalary()) < 0) //first one less than second one it returns -1
                .sorted(Comparator.comparing(Employee::getSalary).reversed())
    ...
    public static Long getSecondMinSalary() throws Exception {
        return getAllEmployees().stream()
                .filter(employee -> employee.getSalary().compareTo(getMinSalary()) > 0) //first one greater than second one it returns 1
                .sorted(Comparator.comparing(Employee::getSalary))
                .findFirst().orElseThrow(() -> new Exception("No employee found!")).getSalary();

//      check if employee's salary is equal to min one then count them
